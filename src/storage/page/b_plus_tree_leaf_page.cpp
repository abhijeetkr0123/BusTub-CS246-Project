 ...
 */
INDEX_TEMPLATE_ARGUMENTS
int B_PLUS_TREE_LEAF_PAGE_TYPE::KeyIndex(const KeyType &key, const KeyComparator &comparator) const { assert(GetSize() >= 0);
...
int B_PLUS_TREE_LEAF_PAGE_TYPE::RemoveAndDeleteRecord(const KeyType &key, const KeyComparator &comparator) { 
  ...
  int n = GetSize();
  for(int i = tarIdx; i < n; i++){
    array[i].first = array[i+1].first;
    array[i].second = array[i+1].second;
  }
  // InsertNodeAfter(ValueAt(tarIdx),pair.first,pair.second);
  // MappingType first_entry=array[tarIdx];
  // array[tarIdx]=pair;
  // array[tarIdx + 1]=first_entry;
  IncreaseSize(-1);
  return GetSize();
 }
