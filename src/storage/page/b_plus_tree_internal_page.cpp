...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::Init(page_id_t page_id, page_id_t parent_id, int max_size) {
  ...
  SetMaxSize(max_size); //minus 1 for first invalid key
}
/*
...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::MoveLastToFrontOf(BPlusTreeInternalPage *recipient,  KeyType &middle_key,
  ...
  IncreaseSize(-1);
  recipient->CopyFirstFrom(pair,buffer_pool_manager,middle_key);
   }
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::CopyFirstFrom(const MappingType &pair, BufferPoolManager *buffer_pool_manager, KeyType &middle_key) {
   assert(GetSize() + 1 < GetMaxSize());
  ...
  // InsertNodeAfter(ValueAt(0),pair.first,pair.second);
  // MappingType first_entry=array[0];
  // array[0]=pair;
  // array[1]=first_entry;
  IncreaseSize(1);
  array[0] = pair;
